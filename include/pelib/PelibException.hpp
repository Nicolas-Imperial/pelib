...
namespace pelib
{
	...
	class PelibException: public std::exception
	{
		public:
			/** Constructor
				@param message Message to display to the screen if the exception is not caught
			**/
			explicit PelibException(const std::string &message);
			/** Constructor
				@param message Message to display to the screen if the exception is not caught
			**/
			explicit PelibException(const char *message);

			/** Destructor **/
			virtual
			~PelibException() throw();
