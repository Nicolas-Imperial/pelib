...
{
	iterator_t(pair_t(MAP_KEY_T, MAP_VALUE_T)) *first, *last, *middle; /** Address where elements are stored **/
};
typedef struct map(MAP_KEY_T, MAP_VALUE_T) map_t(MAP_KEY_T, MAP_VALUE_T);
typedef struct iterator(pair_t(MAP_KEY_T, MAP_VALUE_T)) map_iterator_t(MAP_KEY_T, MAP_VALUE_T);
...
int
pelib_map_insert(MAP_KEY_T, MAP_VALUE_T)(map_t(MAP_KEY_T, MAP_VALUE_T)*, pair_t(MAP_KEY_T, MAP_VALUE_T));
...
/** Compares all elements of two maps one by one
	@return If a1 and a2 have the same number of elements, for the first pair of elements from both maps a1 and a2 that are different, return -1 if the element of a1 is lower than the element of a2, 1 otherwise. Returns 0 if all elements are identical. If a1 has less elements than a2, then returns -1 other wise returns 1.
**/
int
pelib_map_compare(MAP_KEY_T, MAP_VALUE_T)(map_t(MAP_KEY_T, MAP_VALUE_T)* a1, map_t(MAP_KEY_T, MAP_VALUE_T)* a2);

iterator_t(pair_t(MAP_KEY_T, MAP_VALUE_T))*
pelib_map_next(MAP_KEY_T, MAP_VALUE_T)(iterator_t(pair_t(MAP_KEY_T, MAP_VALUE_T))*);
